...
사진 한 장으로 한국어 리뷰 초안을 생성하는 모바일 우선 PWA입니다. 음식, 공간, 제품, 소품 등 어떤 이미지든 업로드하면 AI가 이미지를 분석해 짧게·보통·상세 3가지 길이의 리뷰와 SNS 해시태그를 즉시 제공합니다.
...
리뷰 작성은 반복적이지만, 사진 속 핵심 포인트를 자연스럽게 문장으로 옮기는 일은 생각보다 번거롭습니다. SnapReview는 이 지점을 해결하기 위해 만들어졌습니다. 사용자는 사진만 업로드하면 되고, 서비스는 이미지의 대상·특징·분위기를 읽어 각 플랫폼에 붙여넣기 쉬운 한국어 리뷰와 SNS 해시태그를 생성합니다.
...
- 인스타그램·네이버·카카오에 바로 붙여넣을 수 있는 SNS 해시태그를 자동 생성한다.

### 핵심 가치

- 텍스트 입력보다 사진 업로드 중심의 빠른 UX
- AI 결과를 그대로 쓰지 않고 수정·재생성·길이 선택이 가능한 유연한 편집 경험
- 리뷰와 함께 최대 20개의 SNS 태그를 즉시 복사할 수 있는 원스톱 콘텐츠 생성
...
- 사진 대상·분위기·카테고리를 반영한 해시태그를 최대 20개 자동 생성합니다.
- 네이버·카카오·인스타그램에 바로 붙여넣을 수 있는 한국어 태그를 제공합니다.
- 결과 화면에서 원하지 않는 태그는 삭제하고, 직접 태그를 추가할 수 있습니다.
- 전체 태그를 한 번에 클립보드로 복사할 수 있습니다.
...
→ Gemini Vision 분석 + 리뷰 3종 + SNS 태그 20개 생성
→ receipt_sessions 저장
→ /result에서 태그 편집 · 리뷰 수정/복사/재생성
...
4. 결과 화면에서 AI가 읽은 정보·생성된 리뷰 3종·SNS 태그 20개를 확인합니다.
5. 사용자는 태그를 편집(삭제·추가)하고 전체 태그를 한 번에 복사합니다.
6. 원하는 길이의 리뷰를 수정하거나 재생성한 뒤 복사합니다.
7. 복사된 결과는 히스토리에 저장되어 이후에도 다시 활용할 수 있습니다.
...
<div align="center">
  <table>
    ...
    <tr>
      ...
      <td align="center">리뷰 탭 전환 · 편집<br>SNS 태그 편집 · 복사</td>
...
| AI | `@google/genai` + Gemini 2.5 Flash | 이미지 이해·한국어 리뷰 생성·SNS 태그 생성 작업을 한 번에 처리할 수 있습니다. |
...
Client
  ...
  ├─ app/result/page.tsx        ← 리뷰 편집 + SNS 태그 편집
  ├─ app/history/page.tsx
  ├─ store/receipt.ts
  └─ store/history.ts

Server
  ├─ app/api/process/route.ts   ← 이미지 분석 + 리뷰 + 태그 생성
...
- 비 iOS 환경에서는 `heic2any`로 JPEG 변환 후 캔버스에서 최대 1600px·품질 0.85 압축을 수행합니다.
...
- 이후 Gemini Vision에 이미지와 프롬프트를 함께 전달해 `extracted`(대상·카테고리·포인트·분위기·**태그 20개**)와 `reviews`를 한 번에 생성합니다.
- 응답은 코드 블록 제거 후 JSON 정규식 추출로 파싱합니다.
- 결과는 `receipt_sessions` 테이블에 저장하고, 프런트엔드에는 `sessionId`·`extractedInfo`·`reviews`를 반환합니다.

#### 4. SNS 태그 처리 전략

- 프롬프트에서 `#` 포함 한국어 태그 정확히 20개를 강제합니다.
- `ExtractedInfo.tags: string[]`에 저장되며, 결과 화면에서 `ocrInfo.tags` 상태로 관리됩니다.
- 태그 삭제·추가·전체 복사를 인라인 UI로 처리하며, 최대 20개 제한을 UI 수준에서 강제합니다.
- 전체 태그는 공백으로 join해 클립보드에 복사하므로 SNS에 그대로 붙여넣기할 수 있습니다.
...
- `GEMINI_MODELS` 환경 변수에 쉼표로 구분된 모델 우선순위를 설정합니다.
- quota 초과(429)나 503 UNAVAILABLE 오류 발생 시 다음 모델로 자동 폴백합니다.
- 모든 모델이 실패하면 429 응답을 반환합니다.

### 설계 포인트

- AI 응답은 자유 텍스트가 아니라 구조화된 JSON으로 강제해 UI와 결합하기 쉽게 만들었습니다.
- 업로드부터 결과까지 상태를 서버 세션이 아니라 클라이언트 스토어로 이어 UX 전환 비용을 줄였습니다.
- 태그 편집은 별도 다이얼로그 없이 인라인으로 처리해 모바일에서 빠른 편집 경험을 제공합니다.
...
├─ app/
│  ├─ api/
│  │  ├─ process/route.ts        ← 이미지 분석 + 리뷰 + SNS 태그 생성
│  │  └─ regenerate/route.ts
│  ├─ history/page.tsx
│  ├─ processing/page.tsx
│  ├─ result/page.tsx            ← 리뷰 + 태그 편집 · 복사
│  ├─ globals.css
│  ├─ layout.tsx
│  └─ manifest.ts
├─ components/
...
│  └─ receipt.ts
├─ types/
│  └─ receipt.ts                 ← ExtractedInfo.tags 포함
├─ docs/
│  ├─ PRD.md
│  └─ ROADMAP.md
├─ assets/                       ← 화면 캡처 이미지
...
- `lib/`
  이미지 전처리·스토리지 접근·Supabase 클라이언트·Gemini 폴백·목업 데이터 등 로직성 유틸을 분리합니다.
- `store/`
  업로드 세션과 히스토리 상태를 Zustand로 관리합니다.
- `types/`
  AI 응답과 로컬 저장 스키마의 타입 기준점을 정의합니다. `ExtractedInfo`에 `tags: string[]`이 포함됩니다.
- `docs/`
...
const extracted: ExtractedInfo = {
  ...
  keyDetails:  parsed.extracted?.keyDetails ?? "",
  moodAndContext: parsed.extracted?.moodAndContext ?? "",
  tags: Array.isArray(parsed.extracted?.tags) ? parsed.extracted.tags.slice(0, 20) : [],
};
```

AI 출력이 항상 깔끔한 JSON이라고 가정하지 않고, 마크다운 코드 블록을 제거한 뒤 실제 객체 문자열만 추출합니다. `tags`는 최대 20개로 슬라이스해 클라이언트 상태에 그대로 전달합니다.
...
// 전체 태그 복사: 공백 join으로 SNS에 바로 붙여넣기
const handleTagsCopy = async () => {
  await navigator.clipboard.writeText(ocrInfo.tags.join(" "));
};
```

태그 입력 시 `#` 없이 입력해도 자동으로 붙여줍니다. 복사는 공백으로 이어 붙여 SNS 플랫폼의 태그 파싱 방식과 호환됩니다.
...
로컬 저장소는 간단하지만 무한정 쌓이면 UX와 디버깅 모두 나빠집니다. 최대 50개로 제한해 브라우저 저장 비용을 통제합니다.
...
- 히스토리에 태그 정보도 함께 저장해 과거 게시물 태그를 재사용할 수 있도록 개선할 수 있습니다.
